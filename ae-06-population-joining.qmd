...
population <- read_csv(
  "data/world-pop-2022.csv",
  quote = "\"",       # respect quoted fields
...
We would use "country" as this is the common variable that exists in both data frames.
...
Left_join() because it will keep all rows and columns from population, just adding the matching continent information.
...
population_continents <- population_clean |>
...
population_continent <- population_clean |>
...
I think Asia likely has the highest population and Oceania the lowest.
...
Reorder continents in order of largest first using mutate(), increase size of points and change colour so they stand out, add population numbers at each point using geom_text().
